...
func DialContext(ctx context.Context, target string, opts ...DialOption) (conn *ClientConn, err error) {
	...
	// 这里设置取消的context, 可以调用cc.cancel主动中断dial
	cc.ctx, cc.cancel = context.WithCancel(context.Background())

	// 初始化参数，dialoption.go文件中有各个配置项的初始化方法 调用newFuncDialOption初始化配置
	...
	// 没有设置拨号器就初始化一个 用来调用 rpc 请求
	if cc.dopts.copts.Dialer == nil {
		// 这里封装了一个func放到dialer中，后续建立连接newHTTP2Client的时候会用到
	...
	defer func() {
		select {
		case <-ctx.Done(): // 收到这个channel数据，说明前面超时了（可能是取消或者timeout超时）
	...
	if cc.dopts.resolverBuilder == nil {
		...
		if cc.dopts.resolverBuilder == nil {
			...
			// 走对应默认值
			// etcd中自己定义了endpoint的解析器，最终也是映射为一堆地址
	...
	// schan不为nil 且没有设置serviceConfig 则阻塞等待设置service config.
	...
	// 当dial建立连接后，这个可以动态的修改serviceConfig配置
	if cc.dopts.scChan != nil {
		go cc.scWatcher()
	}
	...
	// 如果设置了这个 ，直到clientConn准备好之前 阻止拨号
...
// clientConn结构体 每个clientConn都对应多个SubConn。
// 基于resolver得到多个SubConn,根据负载均衡得到对应的SubConn.
type ClientConn struct {
	// ctx cancel 用于管理链接的生命周期
	ctx    context.Context
	...
	authority    string
	dopts        dialOptions
	// 用于管理 ClientConn 总体的连接状态
	csMgr *connectivityStateManager

	// resolverWrapper conns balancerWrapper firstResolveEvent 命名解析，负载均衡相关
	balancerBuildOpts balancer.BuildOptions
	// 负载均衡管理器，通过pick来获取合适的连接执行调用
	...
	// 这两个与channelz相关，channelz是内部grpc内部的一些监控信息，看源码的时候可以忽略这部分
...
//等待clientConn resolver 解析地址成功，context超时返回错误
func (cc *ClientConn) waitForResolvedAddrs(ctx context.Context) error {
	// This is on the RPC path, so we use a fast path to avoid the
	// more-expensive "select" below after the resolver has returned once.
	// 创建clientConn的时候，会fire（）fire会赋值以及close(channel)
...
// 这里是获取一个subConn来服务于grpc，通过pick的方式获取到
